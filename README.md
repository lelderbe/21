...
Чтобы проект стал немного понятнее, ниже расскажу про свой путь)
...
За основу я брал алгоритм сортировки вставками (insertion sort). Идея: перебирать элементы и вставлять в нужное место в результирующем стеке. Мне нравится, когда просто. В итоге появился такой план действий:
<details>
  <summary>Спойлер: алгоритм</summary>

1. Перекидываем все элементы в стек B, чтобы стек A оказался пуст. Теперь в стек A будем переносить элементы в отсортированном порядке (но не обязательно начинать с наименьшего).
    
2. Для этого просчитываем для каждого элемента в стеке B стоимость его переноса в стек A (сколько операций необходимо будет произвести с обоими стеками). Выбираем элемент с наименьшей стоимостью и производим действия, необходимые для его переноса (прокрутка стеков, push a).
...
Как можно улучшить: комбинировать операции прокрутки стеков (например: ra и rb заменить на rr), на шаге 1 перебрасывать в массив B с минимальной сортировкой (точнее, группировкой).

</details>

В процессе выполнения проекта познакомился с radix sort - интересная поразрядная сортировка, которой вообще ничего не нужно знать об элементах стека, просто над каждым элементом необходимо произвести нужные действия. С radix можно сдать проект на троечку и при этом написать по минимуму логики.
